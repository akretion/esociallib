...
class TAlvaraJudicial:
    ...
    nrProcJud: Optional[str] = field(
        ...
        metadata={
            "type": "Element",
            "namespace": "http://www.esocial.gov.br/schema/evt/evtRmnRPPS/v_S_01_01_00",
            "required": True,
            "length": 20,
        }
    )
...
class TRemunPer:
    ...
    :ivar matricula: Matrícula atribuída ao trabalhador pela empresa ou,
        ...
        Administração de Recursos Humanos do órgão. Validação: Deve
        corresponder à matrícula informada pelo empregador no evento
        S-2200 ou S-2300 do respectivo contrato. Não preencher no caso
        de Trabalhador Sem Vínculo de Emprego/Estatutário - TSVE sem
        informação de matrícula no evento S-2300 ou, no caso de
        {remunPerAnt}(1202_dmDev_infoPerAnt_idePeriodo_ideEstab_remunPerAnt),
        se {remunOrgSuc}(1202_dmDev_infoPerAnt_remunOrgSuc) = [S].
    :ivar itensRemun:
    """
    class Meta:
        name = "T_remunPer"
    ...
    itensRemun: List[TItensRemunRpps] = field(
        default_factory=list,
        metadata={
            "type": "Element",
            "namespace": "http://www.esocial.gov.br/schema/evt/evtRmnRPPS/v_S_01_01_00",
...
class ESocial:
    ...
    :ivar evtRmnRPPS: Evento Remuneração de Servidor vinculado ao RPPS
        DESCRICAO_COMPLETA:Evento Remuneração de Servidor vinculado ao
        Regime Próprio de Previdência Social. CHAVE_GRUPO: {Id}
    ...
    evtRmnRPPS: Optional["ESocial.EvtRmnRpps"] = field(
        default=None,
        metadata={
            "type": "Element",
            "required": True,
    ...
    class EvtRmnRpps:
        ...
        :ivar ideTrabalhador: Identificação do trabalhador. CHAVE_GRUPO:
            {cpfTrab*}
        :ivar dmDev: Demonstrativo de valores devidos ao trabalhador
        ...
        :ivar Id:
        """
        ideEvento: Optional[TIdeEventoFolhaOpp] = field(
            default=None,
            metadata={
                "type": "Element",
                "required": True,
        ...
        ideTrabalhador: Optional["ESocial.EvtRmnRpps.IdeTrabalhador"] = field(
            default=None,
            metadata={
                "type": "Element",
                "required": True,
        ...
        class IdeTrabalhador:
            """
            :ivar cpfTrab:
            :ivar infoComplem: Informações complementares de
                ...
                informar remuneração devida pelo órgão sucessor a
                servidor desligado ainda no sucedido. No caso das
                categorias em que o evento TSVE - Início for opcional, o
                preenchimento do grupo somente é exigido se não existir
                o respectivo evento. As informações complementares são
                necessárias para correta identificação do trabalhador.
                CONDICAO_GRUPO: O ((se o trabalhador não tiver nenhum
                cadastro no RET) OU (se
                {remunOrgSuc}(1202_dmDev_infoPerAnt_remunOrgSuc) =
            ...
            infoComplem: Optional["ESocial.EvtRmnRpps.IdeTrabalhador.InfoComplem"] = field(
                default=None,
                metadata={
                    "type": "Element",
                }
            ...
            class InfoComplem:
                ...
                sucessaoVinc: Optional["ESocial.EvtRmnRpps.IdeTrabalhador.InfoComplem.SucessaoVinc"] = field(
                    default=None,
                    metadata={
                        "type": "Element",
                    }
                ...
                class SucessaoVinc:
                    ...
                    :ivar cnpjOrgaoAnt: Informar o CNPJ do órgão público
                        ...
                        considerando as particularidades aplicadas à
                        informação de CNPJ de órgão público em S-1000.
                        Além disso, deve possuir 14 (catorze) algarismos
                        e ser diferente do CNPJ base do órgão público
                        declarante (exceto se
                        {ideEmpregador/nrInsc}(1202_ideEmpregador_nrInsc)
                        tiver 14 (catorze) algarismos) e dos
                        estabelecimentos informados através do evento
                        S-1005.
                    :ivar matricAnt: Matrícula do trabalhador no órgão
                        público anterior.
                    :ivar dtExercicio: Preencher com a data de exercício
                        do servidor. No caso de transferência do
                        servidor, deve ser preenchida a data inicial do
                        vínculo no primeiro órgão público (data de
                        início do vínculo).
                    :ivar observacao:
                    """
        ...
        class DmDev:
            """
            :ivar ideDmDev: Identificador atribuído pelo órgão público
                para o demonstrativo de valores devidos ao trabalhador.
                O ente público pode preencher este campo utilizando-se
                de um identificador padrão para todos os trabalhadores;
                no entanto, havendo mais de um demonstrativo relativo a
                uma mesma competência, devem ser utilizados
                identificadores diferentes para cada um dos
                demonstrativos. Validação: Deve ser um identificador
                único dentro do mesmo {perApur}(1202_ideEvento_perApur)
                para cada um dos demonstrativos do trabalhador.
                REGRA:REGRA_CARACTERE_ESPECIAL
            :ivar codCateg:
            :ivar indRRA:
            :ivar infoRRA:
            :ivar infoPerApur: Informações relativas ao período de
                apuração. CONDICAO_GRUPO: O (se não existir o grupo
                {infoPerAnt}(1202_dmDev_infoPerAnt)); OC (nos demais
                casos)
            :ivar infoPerAnt: Informações relativas a períodos
                anteriores DESCRICAO_COMPLETA:Grupo destinado às
                informações de: a) remuneração relativa a diferenças de
                vencimento provenientes de disposições legais; b) verbas
                de natureza salarial ou não salarial devidas após o
                desligamento; c) decisões administrativas ou judiciais
                relativas a diferenças de remuneração. OBS.: As
                informações previstas acima podem se referir ao período
                de apuração definido em
                {perApur}(1202_ideEvento_perApur) ou a períodos
                anteriores a {perApur}(1202_ideEvento_perApur).
                CONDICAO_GRUPO: O (se não existir o grupo
                {infoPerApur}(1202_dmDev_infoPerApur) e
                {indApuracao}(1202_ideEvento_indApuracao) = [1]); N (se
                {indApuracao}(1202_ideEvento_indApuracao) = [2]); OC
                (nos demais casos)
            """
            ...
            infoPerApur: Optional["ESocial.EvtRmnRpps.DmDev.InfoPerApur"] = field(
                default=None,
                metadata={
                    "type": "Element",
                }
            )
            infoPerAnt: Optional["ESocial.EvtRmnRpps.DmDev.InfoPerAnt"] = field(
                default=None,
                metadata={
                    "type": "Element",
                }
            ...
            class InfoPerApur:
                """
                :ivar ideEstab: Identificação da unidade do órgão
                    público DESCRICAO_COMPLETA:Identificação da unidade
                    do órgão público na qual o servidor possui
                    remuneração. CHAVE_GRUPO: {tpInsc}, {nrInsc}
                """
                ...
                class IdeEstab:
                    """
                    :ivar tpInsc:
                    :ivar nrInsc: Informar o número de inscrição da
                        unidade do órgão público.
                    :ivar remunPerApur: Remuneração do trabalhador
            ...
            class InfoPerAnt:
                """
                :ivar remunOrgSuc: Indicar se a remuneração é relativa a
                    verbas de natureza salarial ou não salarial devidas
                    pelo órgão sucessor a servidor desligado ainda no
                    sucedido.
                :ivar idePeriodo: Identificação do período de referência
                    da remuneração DESCRICAO_COMPLETA:Identificação do
                    período ao qual se referem as diferenças de
                    remuneração. CHAVE_GRUPO: {perRef}
                """
                remunOrgSuc: Optional[TsSimNao] = field(
                    default=None,
                    metadata={
                        "type": "Element",
                        "required": True,
                ...
                class IdePeriodo:
                    """
                    :ivar perRef: Informar o período ao qual se refere o
                        complemento de remuneração, no formato AAAA-MM.
                        Validação: Deve ser igual ou anterior ao período
                        de apuração informado em
                        {perApur}(/ideEvento_perApur). Deve ser
                        informado no formato AAAA-MM.
                    :ivar ideEstab: Identificação da unidade do órgão
                        público DESCRICAO_COMPLETA:Identificação da
                        unidade do órgão público na qual o servidor
                        possui remuneração. CHAVE_GRUPO: {tpInsc},
                        {nrInsc}
                    """
                    ...
                    class IdeEstab:
                        ...
                        :ivar nrInsc: Informar o número de inscrição da
                            unidade do órgão público.
                        :ivar remunPerAnt: Remuneração do trabalhador
                            DESCRICAO_COMPLETA:Informações relativas à
                            remuneração do trabalhador em períodos
                        ...
                        remunPerAnt: List[TRemunPer] = field(
                            default_factory=list,
                            metadata={
                                "type": "Element",
                                "min_occurs": 1,
