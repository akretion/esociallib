...
class TAlvaraJudicial:
    ...
    nrProcJud: Optional[str] = field(
        ...
        metadata={
            "type": "Element",
            "namespace": "http://www.esocial.gov.br/schema/evt/evtTabLotacao/v_S_01_01_00",
            "required": True,
            "length": 20,
        }
    )
...
class TIdeLotacao:
    ...
    :ivar codLotacao: Informar o código atribuído pelo empregador para a
        lotação tributária. Validação: O código atribuído não pode
        conter a expressão 'eSocial' nas 7 primeiras posições.
...
class TDadosLotacao:
    ...
    :ivar tpInsc: Preencher com o código correspondente ao tipo de
        inscrição, conforme Tabela 05. Validação: O campo não deve ser
        preenchido se {tpLotacao}(./tpLotacao) for igual a [01, 10, 21,
        24, 90, 91]. Nos demais casos, observar conteúdo exigido para o
        campo {dadosLotacao/nrInsc}(./nrInsc), conforme Tabela 10.
    :ivar nrInsc: Preencher com o número de inscrição (CNPJ, CPF, CNO)
        ao qual pertence a lotação tributária. Validação: a) Deve ser
        preenchido de acordo com o conteúdo exigido, conforme
        especificado no campo {dadosLotacao/tpInsc}(./tpInsc) e na
        Tabela 10; b) Deve ser um identificador válido, constante das
        bases da RFB.
    :ivar fpasLotacao: Informações de FPAS e Terceiros relativos à
        lotação tributária.
    :ivar infoEmprParcial: Informação complementar de obra de construção
        civil DESCRICAO_COMPLETA:Informação complementar que apresenta
        identificação do contratante de obra de construção civil sob
        regime de empreitada parcial ou subempreitada. CONDICAO_GRUPO: O
        (se
        {tpLotacao}(1020_infoLotacao_inclusao_dadosLotacao_tpLotacao) =
        [02]); N (nos demais casos)
    :ivar dadosOpPort: Informações do operador portuário.
        CONDICAO_GRUPO: O (se
        {tpLotacao}(1020_infoLotacao_inclusao_dadosLotacao_tpLotacao) =
    ...
    fpasLotacao: Optional["TDadosLotacao.FpasLotacao"] = field(
        default=None,
        metadata={
            "type": "Element",
            "namespace": "http://www.esocial.gov.br/schema/evt/evtTabLotacao/v_S_01_01_00",
    ...
    infoEmprParcial: Optional["TDadosLotacao.InfoEmprParcial"] = field(
        default=None,
        metadata={
            "type": "Element",
            "namespace": "http://www.esocial.gov.br/schema/evt/evtTabLotacao/v_S_01_01_00",
    ...
    dadosOpPort: Optional["TDadosLotacao.DadosOpPort"] = field(
        default=None,
        metadata={
            "type": "Element",
            "namespace": "http://www.esocial.gov.br/schema/evt/evtTabLotacao/v_S_01_01_00",
    ...
    class FpasLotacao:
        """
        :ivar fpas:
        :ivar codTercs: Preencher com o código de Terceiros, já
            considerando a existência de eventuais convênios para
            recolhimento direto. Ex.: Se o contribuinte está enquadrado
            com FPAS [507], cujo código cheio de Terceiros é [0079], se
            possuir convênio com SENAI deve informar o código [0075].
            Validação: Se a classificação tributária em S-1000 for igual
            a [01, 02, 03, 04], informar [0000]. Nos demais casos, o
            código de Terceiros informado deve ser compatível com o
            código de FPAS informado, conforme Tabela 04.
        :ivar codTercsSusp: Informar o código combinado dos Terceiros
            para os quais o recolhimento está suspenso em virtude de
            processos judiciais. Ex.: Se o contribuinte possui decisões
            de processos para suspensão de recolhimentos ao SESI (0008)
            e ao SEBRAE (0064), deve informar o código combinado das
            duas entidades, ou seja, [0072]. Validação: Deve ser um
            código consistente com a Tabela 04. Deve haver pelo menos um
            processo em
            {procJudTerceiro}(1020_infoLotacao_inclusao_dadosLotacao_fpasLotacao_infoProcJudTerceiros_procJudTerceiro)
            para cada código de Terceiro cujo recolhimento esteja
            suspenso.
        :ivar infoProcJudTerceiros: Informações de processos judiciais
            relativos às contribuições destinadas a Outras Entidades
            DESCRICAO_COMPLETA:Informações sobre a existência de
            processos judiciais, com sentença/decisão favorável ao
            contribuinte, relativos às contribuições destinadas a Outras
            Entidades e Fundos. CONDICAO_GRUPO: O (se
            {codTercsSusp}(1020_infoLotacao_inclusao_dadosLotacao_fpasLotacao_codTercsSusp)
        ...
        infoProcJudTerceiros: Optional["TDadosLotacao.FpasLotacao.InfoProcJudTerceiros"] = field(
            default=None,
            metadata={
                "type": "Element",
                "namespace": "http://www.esocial.gov.br/schema/evt/evtTabLotacao/v_S_01_01_00",
        ...
        @dataclass
        class InfoProcJudTerceiros:
            """
            :ivar procJudTerceiro: Identificação do processo judicial.
            ...
            class ProcJudTerceiro:
                """
                :ivar codTerc: Informar o código de Terceiro. Validação:
                    Deve ser um código de Terceiro válido e compatível
                    com o FPAS/Terceiros informado no grupo superior,
                    conforme Tabela 04.
                :ivar nrProcJud:
                :ivar codSusp:
                """
    ...
    class InfoEmprParcial:
        ...
        :ivar nrInscContrat: Número de inscrição (CNPJ/CPF) do
            contratante. Validação: Deve ser um número de CNPJ ou CPF
            válido, conforme definido em
            {tpInscContrat}(./tpInscContrat).
        :ivar tpInscProp: Tipo de inscrição do proprietário do CNO.
            Validação: Preenchimento obrigatório e exclusivo quando o
            proprietário não for encontrado no CNO.
        :ivar nrInscProp: Preencher com o número de inscrição (CNPJ/CPF)
            do proprietário do CNO. Validação: Preenchimento obrigatório
            e exclusivo se {tpInscProp}(./tpInscProp) for informado.
            Deve ser um número de CNPJ ou CPF válido, conforme indicado
            em {tpInscProp}(./tpInscProp).
        """
    ...
    class DadosOpPort:
        """
        :ivar aliqRat: Preencher com a alíquota definida na legislação
            vigente para a atividade (CNAE) preponderante.
        :ivar fap: Fator Acidentário de Prevenção - FAP. Validação: Deve
            ser um número maior ou igual a 0,5000 e menor ou igual a
            2,0000, de acordo com o estabelecido pelo órgão
...
class ESocial:
    ...
    :ivar evtTabLotacao: Evento Tabela de Lotações Tributárias.
    ...
    evtTabLotacao: Optional["ESocial.EvtTabLotacao"] = field(
        default=None,
        metadata={
            "type": "Element",
            "required": True,
    ...
    class EvtTabLotacao:
        ...
        :ivar ideEvento:
        :ivar ideEmpregador:
        :ivar infoLotacao: Informações da lotação
        ...
        infoLotacao: Optional["ESocial.EvtTabLotacao.InfoLotacao"] = field(
            default=None,
            metadata={
                "type": "Element",
                "required": True,
        ...
        class InfoLotacao:
            ...
            """
            inclusao: Optional["ESocial.EvtTabLotacao.InfoLotacao.Inclusao"] = field(
                default=None,
                metadata={
                    "type": "Element",
                }
            )
            alteracao: Optional["ESocial.EvtTabLotacao.InfoLotacao.Alteracao"] = field(
                default=None,
                metadata={
                    "type": "Element",
                }
            )
            exclusao: Optional["ESocial.EvtTabLotacao.InfoLotacao.Exclusao"] = field(
                default=None,
                metadata={
                    "type": "Element",
                }
            ...
            @dataclass
            class Inclusao:
                ideLotacao: Optional[TIdeLotacao] = field(
                    default=None,
                    metadata={
                        "type": "Element",
                        "required": True,
                ...
                dadosLotacao: Optional[TDadosLotacao] = field(
                    default=None,
                    metadata={
                        "type": "Element",
                        "required": True,
