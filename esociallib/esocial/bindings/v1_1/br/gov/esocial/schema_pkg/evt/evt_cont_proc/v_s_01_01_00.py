...
class TAlvaraJudicial:
    ...
    nrProcJud: Optional[str] = field(
        ...
        metadata={
            "type": "Element",
            "namespace": "http://www.esocial.gov.br/schema/evt/evtContProc/v_S_01_01_00",
            "required": True,
            "length": 20,
        }
    )
...
class InfoCrirrfTpCr(Enum):
    ...
    :cvar VALUE_188951: IRRF - RRA - Decisão da Justiça do Trabalho
    """
    VALUE_593656 = 593656
    VALUE_188951 = 188951
...
class ESocial:
    ...
    :ivar evtContProc: Evento Informações de Tributos Decorrentes de
        Processo Trabalhista. CHAVE_GRUPO: {Id}
    ...
    evtContProc: Optional["ESocial.EvtContProc"] = field(
        default=None,
        metadata={
            "type": "Element",
            "required": True,
    ...
    class EvtContProc:
        ...
        :ivar ideProc: Identificação do processo. CHAVE_GRUPO:
            {nrProcTrab*}, {perApurPgto*}
        :ivar ideTrab: Identificação do trabalhador. CHAVE_GRUPO:
            {cpfTrab}
        :ivar Id:
        """
        ...
        ideEmpregador: Optional["ESocial.EvtContProc.IdeEmpregador"] = field(
            default=None,
            metadata={
                "type": "Element",
                "required": True,
        ...
        ideProc: Optional["ESocial.EvtContProc.IdeProc"] = field(
            default=None,
            metadata={
                "type": "Element",
                "required": True,
        ...
        class IdeProc:
            ...
            :ivar nrProcTrab: Número do processo trabalhista, da ata ou
                número de identificação da conciliação. Validação: Deve
                ser um número de processo válido e declarado no evento
                S-2500 para o(s) trabalhador(es) informado(s) em
                {cpfTrab}(2501_ideTrab_cpfTrab).
            :ivar perApurPgto: Mês/ano em que é devida a obrigação de
                pagar a parcela prevista no acordo/sentença. Validação:
                Deve ser um período igual ou posterior ao mês/ano de
                {dtSent}(2500_infoProcesso_dadosCompl_infoProcJud_dtSent)
                ou de
                {dtCCP}(2500_infoProcesso_dadosCompl_infoCCP_dtCCP)
                existente no evento S-2500 para o processo indicado em
                {nrProcTrab}(./nrProcTrab). Deve ser informado no
                formato AAAA-MM.
            :ivar obs: Observação referente ao pagamento de parcela
                prevista no acordo/sentença.
            """
            ...
            perApurPgto: Optional[str] = field(
                default=None,
                metadata={
                    "type": "Element",
                    "required": True,
        ...
        class IdeTrab:
            """
            :ivar calcTrib: Identificação do período e da base de
                cálculo dos tributos. CHAVE_GRUPO: {perRef}
            :ivar infoCRIRRF: Informações de IRRF.
                DESCRICAO_COMPLETA:Informações de Imposto de Renda
                Retido na Fonte, por Código de Receita - CR.
                CHAVE_GRUPO: {tpCR} CONDICAO_GRUPO: OC
            :ivar cpfTrab: Preencher com o número do CPF do trabalhador.
                Validação: Deve ser um CPF válido e informado no evento
                S-2500.
            """
            ...
            class CalcTrib:
                ...
                :ivar infoCRContrib: Informações das contribuições
                    sociais. DESCRICAO_COMPLETA:Informações das
                    contribuições sociais devidas à Previdência Social e
                    Outras Entidades e Fundos, por Código de Receita -
                    CR. CHAVE_GRUPO: {tpCR} CONDICAO_GRUPO: OC
                :ivar perRef: Informar o mês/ano (formato AAAA-MM) de
                    referência das informações. Validação: Deve ser um
                    período existente no evento S-2500 para o
                    trabalhador indicado em {cpfTrab}(../cpfTrab) e
                    igual ou posterior a [2008-12].
                :ivar vrBcCpMensal: Valor da base de cálculo da
                    contribuição previdenciária sobre a remuneração
                    mensal do trabalhador. Validação: Deve ser maior ou
                    igual a 0 (zero).
                :ivar vrBcCp13: Valor da base de cálculo da contribuição
                    previdenciária sobre a remuneração do trabalhador
                    referente ao 13º salário. Validação: Deve ser maior
                    ou igual a 0 (zero).
                :ivar vrRendIRRF: Valor do rendimento tributável do
                    Imposto de Renda. Validação: Deve ser maior ou igual
                    a 0 (zero).
                :ivar vrRendIRRF13: Valor do rendimento tributável do
                    Imposto de Renda referente ao 13º salário -
                    Tributação exclusiva. Validação: Deve ser maior ou
                    igual a 0 (zero).
                """
                ...
                vrRendIRRF: Optional[Decimal] = field(
                    default=None,
                    metadata={
                        "type": "Attribute",
                        "required": True,
                ...
                vrRendIRRF13: Optional[Decimal] = field(
                    default=None,
                    metadata={
                        "type": "Attribute",
                        "required": True,
                ...
                class InfoCrcontrib:
                    """
                    :ivar tpCR: Código de Receita - CR relativo a
                        contribuições sociais devidas à Previdência
                        Social e a Outras Entidades e Fundos
                        (Terceiros), conforme legislação em vigor na
                        competência. Validação: Deve ser um código
                        válido e existente na Tabela 29.
                    :ivar vrCR: Valor correspondente ao Código de
                        Receita - CR. Validação: Deve ser informado de
            ...
            class InfoCrirrf:
                """
                :ivar tpCR: Código de Receita - CR relativo a Imposto de
                    Renda Retido na Fonte.
                :ivar vrCR: Valor correspondente ao Código de Receita -
                    CR. Validação: Deve ser informado de acordo com a
                    legislação em vigor na competência. Deve ser maior
                    que 0 (zero).
                """
                tpCR: Optional[InfoCrirrfTpCr] = field(
                    default=None,
                    metadata={
                        "type": "Attribute",
                        "required": True,
