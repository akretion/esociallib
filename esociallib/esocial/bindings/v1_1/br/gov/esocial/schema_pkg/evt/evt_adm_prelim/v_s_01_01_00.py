...
class TAlvaraJudicial:
    ...
    nrProcJud: Optional[str] = field(
        ...
        metadata={
            "type": "Element",
            "namespace": "http://www.esocial.gov.br/schema/evt/evtAdmPrelim/v_S_01_01_00",
            "required": True,
            "length": 20,
        }
    )
...
class ESocial:
    ...
    :ivar evtAdmPrelim: Evento Registro Preliminar de Trabalhador.
        CHAVE_GRUPO: {Id} REGRA:REGRA_ADMISSAO_VALIDA_DT_ADM
    ...
    evtAdmPrelim: Optional["ESocial.EvtAdmPrelim"] = field(
        default=None,
        metadata={
            "type": "Element",
            "required": True,
    ...
    class EvtAdmPrelim:
        ...
        :ivar ideEvento:
        :ivar ideEmpregador:
        :ivar infoRegPrelim: Informações do registro preliminar do
            trabalhador. CHAVE_GRUPO: {cpfTrab*}, {matricula*}
        :ivar Id:
        """
        ...
        infoRegPrelim: Optional["ESocial.EvtAdmPrelim.InfoRegPrelim"] = field(
            default=None,
            metadata={
                "type": "Element",
                "required": True,
        ...
        class InfoRegPrelim:
            ...
            :ivar cpfTrab:
            :ivar dtNascto: Preencher com a data de nascimento.
            :ivar dtAdm: Preencher com a data de admissão do trabalhador
                (ou data de início, no caso de Trabalhador Sem Vínculo
                de Emprego/Estatutário - TSVE). Validação: Deve ser
                posterior à data de nascimento do trabalhador e igual ou
                posterior à data de início da obrigatoriedade dos
                eventos não periódicos para o empregador.
            :ivar matricula: Matrícula atribuída ao trabalhador pela
                empresa. Validação: O valor informado não pode conter a
            ...
            :ivar codCateg: Preencher com o código da categoria do
                trabalhador. Validação: Deve ser um código de categoria
                sujeito ao Registro de Eventos Trabalhistas - RET de
                trabalhador não vinculado ao Regime Próprio de
                Previdência Social - RPPS, ou seja, "Empregado" ([1XX]),
                algumas categorias de "Agente Público" ([301, 302, 303,
                304, 306, 307, 309, 310, 312]), "Avulso" ([2XX]),
                "Cessão" ([4XX]), algumas categorias de "Contribuinte
                Individual" ([721, 722, 723, 731, 734, 738, 761, 771])
                ou de "Bolsista" ([901, 902]).
            :ivar natAtividade: Natureza da atividade. Validação:
                Preenchimento obrigatório se {codCateg}(./codCateg) for
                relativo a "Empregado", "Agente Público", "Avulso" ou
                igual a [401, 731, 734, 738]. Não deve ser preenchido se
                {codCateg}(./codCateg) = [721, 722, 771, 901]. Se
                {codCateg}(./codCateg) = [104], deve ser preenchido com
                [1]. Se {codCateg}(./codCateg) = [102], deve ser
                preenchido com [2].
            :ivar infoRegCTPS: Informações referentes ao registro e à
                CTPS Digital DESCRICAO_COMPLETA:Informações referentes
                ao registro eletrônico de empregados e à Carteira de
                Trabalho e Previdência Digital - CTPS Digital.
            ...
            infoRegCTPS: Optional["ESocial.EvtAdmPrelim.InfoRegPrelim.InfoRegCtps"] = field(
                default=None,
                metadata={
                    "type": "Element",
                }
            ...
            class InfoRegCtps:
                """
                :ivar CBOCargo: Informar a Classificação Brasileira de
                    Ocupações - CBO relativa ao cargo. Validação: Deve
                    ser um código válido e existente na tabela de CBO,
                    com 6 (seis) posições.
                :ivar vrSalFx:
                :ivar undSalFixo:
                :ivar tpContr:
                :ivar dtTerm: Data do término do contrato por prazo
                    determinado. Validação: O preenchimento é
                    obrigatório se {tpContr}(./tpContr) = [2]. Não
                    informar se {tpContr}(./tpContr) = [1]. Deve ser
