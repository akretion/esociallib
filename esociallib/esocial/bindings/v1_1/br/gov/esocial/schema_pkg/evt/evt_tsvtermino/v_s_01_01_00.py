...
class TAlvaraJudicial:
    ...
    nrProcJud: Optional[str] = field(
        ...
        metadata={
            "type": "Element",
            "namespace": "http://www.esocial.gov.br/schema/evt/evtTSVTermino/v_S_01_01_00",
            "required": True,
            "length": 20,
        }
    )
...
class RemunAposTermIndRemun(Enum):
    ...
    :cvar VALUE_2: Término reconhecido judicialmente com data anterior a
...
class ESocial:
    ...
    :ivar evtTSVTermino: Evento TSVE - Término DESCRICAO_COMPLETA:Evento
    ...
    evtTSVTermino: Optional["ESocial.EvtTsvtermino"] = field(
        default=None,
        metadata={
            "type": "Element",
            "required": True,
    ...
    class EvtTsvtermino:
        ...
        ideTrabSemVinculo: Optional[TIdeTrabSemVinculo] = field(
            default=None,
            metadata={
                "type": "Element",
                "required": True,
        ...
        infoTSVTermino: Optional["ESocial.EvtTsvtermino.InfoTsvtermino"] = field(
            default=None,
            metadata={
                "type": "Element",
                "required": True,
        ...
        class InfoTsvtermino:
            """
            :ivar dtTerm: Data do término. Validação: Deve ser uma data
                igual ou anterior à data atual acrescida de 10 (dez)
                dias.
            :ivar mtvDesligTSV:
            :ivar pensAlim: Indicativo de pensão alimentícia para fins
                de retenção de FGTS. Validação: Informação obrigatória
                se o código de categoria no RET for igual a [201, 202,
                721] e se {dtTerm}(./dtTerm) for posterior a 21/04/2019.
                Informação opcional se o código de categoria no RET for
                igual a [201, 202, 721] e se {dtTerm}(./dtTerm) for
                igual ou anterior a 21/04/2019. Não preencher nas demais
                situações.
            :ivar percAliment:
            :ivar vrAlim:
            :ivar nrProcTrab: Número que identifica o processo
                trabalhista, quando o término de TSVE se der por decisão
            ...
            :ivar mudancaCPF: Informação do novo CPF do trabalhador.
                CONDICAO_GRUPO: O (se
                {mtvDesligTSV}(2399_infoTSVTermino_mtvDesligTSV) =
                [07]); N (nos demais casos)
            :ivar verbasResc: Verbas rescisórias
                DESCRICAO_COMPLETA:Grupo onde são prestadas as
                informações relativas às verbas rescisórias do diretor
                não empregado, com FGTS. CONDICAO_GRUPO: N (se
                {mtvDesligTSV}(2399_infoTSVTermino_mtvDesligTSV) = [07]
                OU {dtTerm}(2399_infoTSVTermino_dtTerm) for anterior ao
                início de obrigatoriedade dos eventos periódicos para o
                empregador OU o código de categoria no RET for diferente
                de [721]); OC (nos demais casos)
            :ivar remunAposTerm: Informações sobre a quarentena
                remunerada ou outra situação de término com data
                anterior DESCRICAO_COMPLETA:Informações sobre a
                "quarentena" remunerada de trabalhador desligado ou
                outra situação de término com data anterior. O grupo
                deve ser preenchido apenas no caso do trabalhador que
                recebe remuneração após o desligamento por estar
                impossibilitado de exercer atividade remunerada ou no
                caso de término reconhecido judicialmente com data
                anterior a competências com remunerações já informadas
                no eSocial. CONDICAO_GRUPO: OC
            """
            ...
            mtvDesligTSV: Optional[InfoTsvterminoMtvDesligTsv] = field(
                default=None,
                metadata={
                    "type": "Element",
                }
            ...
            mudancaCPF: Optional["ESocial.EvtTsvtermino.InfoTsvtermino.MudancaCpf"] = field(
                default=None,
                metadata={
                    "type": "Element",
                }
            )
            verbasResc: Optional["ESocial.EvtTsvtermino.InfoTsvtermino.VerbasResc"] = field(
                default=None,
                metadata={
                    "type": "Element",
                }
            )
            remunAposTerm: Optional["ESocial.EvtTsvtermino.InfoTsvtermino.RemunAposTerm"] = field(
                default=None,
                metadata={
                    "type": "Element",
                }
            ...
            class VerbasResc:
                ...
                class DmDev:
                    ...
                    :ivar ideDmDev: Identificador atribuído pela empresa
                        ...
                        término) para cada um dos demonstrativos do
                    ...
                    :ivar infoRRA:
                    :ivar ideEstabLot: Identificação do estabelecimento
                        e lotação DESCRICAO_COMPLETA:Identificação do
                        estabelecimento e da lotação nos quais o
                        trabalhador possui remuneração no período de
                        apuração. CHAVE_GRUPO: {tpInsc}, {nrInsc},
                        {codLotacao}
                    """
                    ...
                    class IdeEstabLot:
                        ...
                        :ivar nrInsc: Informar o número de inscrição do
                            estabelecimento do contribuinte de acordo
                            com o tipo de inscrição indicado no campo
                            {ideEstabLot/tpInsc}(./tpInsc). Validação: A
                            inscrição informada deve ser compatível com
                            {ideEstabLot/tpInsc}(./tpInsc).
                        :ivar codLotacao:
                        :ivar detVerbas: Detalhamento das verbas
                            rescisórias DESCRICAO_COMPLETA:Detalhamento
                            das verbas rescisórias devidas ao
                            trabalhador. Deve haver pelo menos uma
                            rubrica de folha, mesmo que o valor líquido
                            a ser pago ao trabalhador seja 0 (zero) em
                            função de descontos.
                        :ivar infoSimples:
                        """
                        tpInsc: Optional[TsTpInscEstabLot14] = field(
                            default=None,
                            metadata={
                                "type": "Element",
                                "required": True,
                        ...
                        detVerbas: List["ESocial.EvtTsvtermino.InfoTsvtermino.VerbasResc.DmDev.IdeEstabLot.DetVerbas"] = field(
                            default_factory=list,
                            metadata={
                                "type": "Element",
                                "min_occurs": 1,
                        ...
                        class DetVerbas:
                            """
                            :ivar codRubr: Informar o código atribuído
                                pelo empregador que identifica a rubrica
                                em sua folha de pagamento ou o código da
                                rubrica constante da Tabela de Rubricas
                                Padrão. Validação: Não pode ser
                                utilizada rubrica: a) cujo
                                {codIncCP}(1010_infoRubrica_inclusao_dadosRubrica_codIncCP)
                                em S-1010 seja igual a [25, 26, 51]; b)
                                cuja
                                {natRubr}(1010_infoRubrica_inclusao_dadosRubrica_natRubr)
                                em S-1010 seja igual a [1801, 9220],
                                desde que mês/ano de
                                {dtTerm}(2399_infoTSVTermino_dtTerm)
                                &gt;= [2021-07].
                            :ivar ideTabRubr:
                            :ivar qtdRubr:
                            :ivar fatorRubr:
                            :ivar vrRubr:
                            :ivar indApurIR: Indicativo de tipo de
                                apuração de IR. Validação: Informação
            ...
            class RemunAposTerm:
                ...
                :ivar dtFimRemun: Preencher com a data final da
                    ...
                    de término reconhecido judicialmente com data
                    ...
                    {dtTerm}(2399_infoTSVTermino_dtTerm).
                """
                indRemun: Optional[RemunAposTermIndRemun] = field(
                    default=None,
                    metadata={
                        "type": "Element",
                    }
